...
def AbsPath(path):
    return p2s(Path(path).absolute())


def Folder(path):
    parts = p2s(path).split('/'); return ('/'.join(parts[:-1])+'/') if len(parts)>1 else './'
    
def File(path):
    return p2s(path).split('/')[-1]

def Prefix(path):
    parts = p2s(path).split('.'); return '.'.join(parts[:-1])
...
def AddFormat(path, format):
    return p2s(path) + '.' + format


def Parent(path):
    parts = p2s(path).split('/')
    if parts[-1]=='':
        parts = parts[-1]
    return ('/'.join(parts[:-1])+'/') if len(parts)>1 else './'
    
def IsFolder(path):
    path = p2s(path); return path==''  or path[-1]=='/'

def IsFile(path):
    path = p2s(path); return path!='' and path[-1]!='/'
...
def ListDir(path, ordered=False, with_path=False, **sort_args):
    ...
    return [p2s(Path(path)/Path(obj)) for obj in objs] if with_path else objs

def ListFolders(path, ordered=False, with_path=False, **sort_args):
    objs = sorted([d for d in os.listdir(path) if ExistFolder(p2s(Path(path)/Path(d)))],**sort_args) if ordered else [d for d in os.listdir(path) if ExistFolder(p2s(Path(path)/Path(d)))]
    return [p2s(Path(path)/Path(obj)) for obj in objs] if with_path else objs

def ListFiles(path, ordered=False, with_path=False, **sort_args):
    objs = sorted([d for d in os.listdir(path) if ExistFile(p2s(Path(path)/Path(d)))],**sort_args) if ordered else [d for d in os.listdir(path) if ExistFile(p2s(Path(path)/Path(d)))]
    return [p2s(Path(path)/Path(obj)) for obj in objs] if with_path else objs

def NewestDir(path, with_path=False, topk=1):
    key = (lambda x:os.path.getctime(x)) if with_path else (lambda x:os.path.getctime(p2s(Path(path)/Path(x))))
...
def LoadPickle(path):
    assert (ExistFile(path)), "path '{0}' does not exist".format(path)
    return pickle.load(open(p2s(path),'rb'))
